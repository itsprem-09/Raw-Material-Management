...
class _EditCompositionDialogState extends State<EditCompositionDialog> {
  ...
  late final List<MaterialComposition> _materials;
  final Map<String, TextEditingController> _quantityControllers = {};

  @override
  void initState() {
    super.initState();
    _productNameController = TextEditingController(text: widget.composition.productName);
    _materials = List.from(widget.composition.materials);
  ...
  void _submitForm() {
    if (_formKey.currentState?.validate() ?? false) {
      final updatedComposition = widget.composition.copyWith(
        productName: _productNameController.text,
        materials: _materials,
        updatedAt: DateTime.now(),
      );

      context.read<CompositionBloc>().add(UpdateComposition(updatedComposition));
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Edit Composition'),
      content: Form(
        key: _formKey,
